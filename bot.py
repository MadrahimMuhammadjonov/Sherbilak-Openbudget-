...
# ============================================================
# SOZLAMALAR — .env yoki to'g'ridan-to'g'ri o'zgartiring
# ============================================================
BOT_TOKEN = os.getenv("BOT_TOKEN", "8769316813:AAFEnjpQVcJXiPihOsvZp-06GUJ2bEovrUA")

# Ovoz beriladigan loyiha ma'lumotlari
...
INITIATIVE_INFO_URL = f"{API_BASE}/boards/{BOARD_ID}/initiatives/{INITIATIVE_ID}"
...
class OpenBudgetAPI:
    ...
    def __init__(self, session: aiohttp.ClientSession):
        ...
        self.headers = {
            ...
            "Referer": f"{BASE_URL}/boards/initiatives/initiative/{BOARD_ID}/{INITIATIVE_ID}",
            "Origin": BASE_URL,
        }

    async def get_initiative_info(self) -> dict | None:
        """Loyiha ma'lumotlarini olish"""
        try:
            async with self.session.get(
                INITIATIVE_INFO_URL, headers=self.headers, timeout=aiohttp.ClientTimeout(total=10)
            ) as resp:
                if resp.status == 200:
                    return await resp.json()
        except Exception as e:
            logger.error(f"get_initiative_info xatosi: {e}")
        return None

    async def get_captcha(self) -> tuple[str | None, bytes | None]:
        """Captcha olish — (captcha_id, image_bytes)"""
        try:
            async with self.session.get(
                CAPTCHA_URL, headers=self.headers, timeout=aiohttp.ClientTimeout(total=10)
            ) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    captcha_id = data.get("id") or data.get("captchaId") or data.get("key")
                    # Rasm base64 yoki URL bo'lishi mumkin
                    img_data = data.get("image") or data.get("captchaImage") or data.get("img")
                    if img_data:
                        if img_data.startswith("data:image"):
                            img_data = img_data.split(",", 1)[1]
                        img_bytes = base64.b64decode(img_data)
                        return captcha_id, img_bytes
                    # Yoki to'g'ridan-to'g'ri binary response
    ...
    async def send_otp(self, phone: str, captcha_id: str, captcha_answer: str) -> dict:
        """SMS OTP yuborish"""
        payload = {
            "phone": phone,
    ...
    async def verify_otp(self, phone: str, otp_code: str, session_token: str = "") -> dict:
...
# Global HTTP session (bot lifecycle davomida yashaydi)
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    info_text = (
        ...
        f"🔗 <b>Havola:</b> <a href='https://openbudget.uz/boards/initiatives/initiative/{BOARD_ID}/{INITIATIVE_ID}'>Loyihani ko'rish</a>\n\n"
        "✅ Bu bot orqali siz <b>openbudget.uz</b> saytida ushbu loyihaga ovoz berishingiz mumkin.\n\n"
        "📋 <b>Jarayon:</b>\n"
    ...
    await message.answer(info_text, parse_mode=ParseMode.HTML, reply_markup=phone_btn)
    await message.answer(
        "👇 Quyidagi tugmani bosing yoki telefon raqamingizni qo'lda kiriting:\n"
        "<i>Misol: +998901234567 yoki 901234567</i>",
        parse_mode=ParseMode.HTML
    )
    await state.set_state(VoteStates.waiting_phone)
    logger.info(f"User {user.id} ({user.username}) /start bosdi")
...
async def handle_phone(message: Message, state: FSMContext):
    ...
    img_buf = BytesIO(captcha_img)
    img_buf.name = "captcha.png"
    ...
    logger.info(f"Captcha yuborildi: user={message.from_user.id}, phone={phone}")


# Captcha javobini qabul qilish
...
async def handle_captcha(message: Message, state: FSMContext):
    ...
    logger.info(f"send_otp natija: user={message.from_user.id}, status={status}, data={resp_data}")

    if status in (200, 201):
        # Muvaffaqiyatli — OTP yuborildi
        session_token = resp_data.get("token") or resp_data.get("sessionId") or ""
        ...
        error_msg = resp_data.get("message") or resp_data.get("error") or "Noto'g'ri captcha"
        if "captcha" in str(error_msg).lower():
            # Captcha noto'g'ri — yangi captcha berish
            ...
            if captcha_img_new:
                ...
                await message.answer("❌ Captcha noto'g'ri va yangi captcha yuklab bo'lmadi. /start")
                await state.clear()
        else:
            await message.answer(
...
# OTP kodini qabul qilish
@router.message(VoteStates.waiting_otp)
async def handle_otp(message: Message, state: FSMContext):
    if not message.text:
    ...
    if len(otp_code) < 4:
        await message.answer("❌ Kod kamida 4 ta raqamdan iborat bo'lishi kerak!")
        return
    ...
    logger.info(f"verify_otp natija: user={message.from_user.id}, status={status}, data={resp_data}")
    ...
    if status in (200, 201):
        ...
        await message.answer(
            ...
            f"🔗 <a href='https://openbudget.uz/boards/initiatives/initiative/{BOARD_ID}/{INITIATIVE_ID}'>Natijalarni ko'rish</a>\n\n"
        ...
        error_msg = resp_data.get("message") or resp_data.get("error") or "Noto'g'ri kod"
        if "expired" in str(error_msg).lower() or "eskirgan" in str(error_msg).lower():
            await message.answer(
            ...
            return  # State o'zgartirilmaydi — qayta kiritish uchun
        ...
        await message.answer(
            ...
            f"Sabab: {resp_data.get('message', 'Noma\\'lum xatolik')}\n\n"
            "Qayta urinish: /start",
            parse_mode=ParseMode.HTML
        )
...
async def handle_unknown(message: Message, state: FSMContext):
    ...
    if current_state is None:
        await message.answer(
            "👋 Ovoz berish uchun /start bosing!",
        )
...
async def main():
    ...
    connector = aiohttp.TCPConnector(
        limit=200,           # Max parallel connections
        limit_per_host=50,   # Per host limit
        ssl=False            # SSL verify off (agar kerak bo'lsa True qiling)
    )
